...
export default function KvalitaPage() {
          ...
          <div className="flex flex-col lg:flex-row gap-8 items-start">
            <div className="flex-grow">
              ...
              <p className="text-lg text-earth-700 mb-6">
                ...
                Data jsou aktualizována {dataMetadata.updateFrequency}.
              </p>
                      ...
                      {source.servesAreas.length > 5 && (
                        ...
                        >
                          {expandedSource === source.id ? (
                            ...
                            <>
                              +{source.servesAreas.length - 5} dalších
        ...
        <section className="container-custom py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <ParameterCard
              ...
              status="good"
              description="Fe"
              limit={healthLimits.iron}
            />
            <ParameterCard
              icon={Thermometer}
              label="Mangan"
              value={formatNumber(parameters.manganese, 2)}
              unit="mg/l"
              status="good"
              description="Mn"
              limit={healthLimits.manganese}
            />
            <ParameterCard
              icon={Waves}
              label="Vápník"
              ...
              status="good"
              description="Ca"
              limit={healthLimits.calcium}
            />
            <ParameterCard
              icon={Waves}
              label="Hořčík"
              value={formatNumber(parameters.magnesium)}
              unit="mg/l"
              status="good"
              description="Mg"
              limit={healthLimits.magnesium}
            />
            <ParameterCard
              icon={Beaker}
              label="Chloridy"
              value={formatNumber(parameters.chlorides)}
              unit="mg/l"
              status="good"
              description="Cl⁻"
              limit={{ max: 250, optimal: [0, 100] }}
            />
            <ParameterCard
              icon={Beaker}
              label="Sírany"
              value={formatNumber(parameters.sulfates)}
              unit="mg/l"
              status="good"
              description="SO₄²⁻"
              limit={{ max: 250, optimal: [0, 150] }}
            />
          </div>
        </section>
        ...
        <section className="container-custom py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <ParameterCard
              ...
              limit={{ max: 0, optimal: [0, 0] }}
            />
            <ParameterCard
              icon={AlertCircle}
              label="Enterokoky"
              value={bacteriological.enterococci}
              unit="KTJ/100ml"
              status="good"
              description="Intestinální enterokoky"
              limit={{ max: 0, optimal: [0, 0] }}
            />
            <ParameterCard
              icon={AlertCircle}
              label="Koliformní bakterie"
              value={bacteriological.coliformBacteria}
              unit="KTJ/100ml"
              status="good"
              description="Celkové koliformní bakterie"
              limit={{ max: 0, optimal: [0, 0] }}
            />
          </div>
        </section>
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-water-50">
                  <tr>
                    <th className="px-4 py-3 text-left text-sm font-mono font-bold text-earth-900">
                      Datum
                    </th>
                    <th className="px-4 py-3 text-right text-sm font-mono font-bold text-earth-900">
                      Tvrdost (°dH)
                    </th>
                    <th className="px-4 py-3 text-right text-sm font-mono font-bold text-earth-900">
                      pH
                ...
                <tbody className="divide-y divide-water-100">
                  {historicalData.map((row, index) => (
                    <tr key={index} className="hover:bg-water-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-sm text-right font-mono text-earth-900">
                        {formatNumber(row.hardness)}
                      </td>
                      <td className="px-4 py-3 text-sm text-right font-mono text-earth-900">
                        {formatNumber(row.pH, 1)}
                      </td>
                      <td className="px-4 py-3 text-sm text-right font-mono text-earth-900">
                        {formatNumber(row.nitrates)}
                      </td>
        ...
        <section className="container-custom pb-16">
          <div className="glass-card p-6 bg-water-50/50">
            <div className="flex gap-4">
              ...
              <div>
                <h3 className="font-mono font-bold text-earth-900 mb-2">
                  O datech
                </h3>
                <p className="text-sm text-earth-700 mb-3">
                  Data o kvalitě vody jsou získávána z veřejně dostupných zdrojů
                  a aktualizována {dataMetadata.updateFrequency}.
                </p>
                <div className="flex flex-wrap gap-2">
                  {dataMetadata.sources.map((source) => (
                    <a
                      key={source.url}
                      href={source.url}
                      target="_blank"
